...
class App extends Component {
  state = {
    whatsIsClicked: 'FRUIT',
    mainScreenImg: imagesList[0].imageUrl,
    score: 0,
    isGameRunning: true,
    sec: 60,
  }
  ...
  componentDidMount() {
    this.timerId = setInterval(this.updateSec, 1000)
  }
  ...
  imageClicked = id => {
    const {mainScreenImg} = this.state
    if (id === mainScreenImg) {
    ...
    this.setState({
      mainScreenImg: imagesList[getRandomNum].imageUrl,
    })
  }

  clickedTab = id => {
    const {whatsIsClicked} = this.state
    this.setState({whatsIsClicked: id})
  }
  ...
  renderPage() {
    ...
    const searchRes = imagesList.filter(img => img.category === whatsIsClicked)
    if (isGameRunning === true) {
      return (
        <div className="bg-container">
          ...
          <div className="image-res-container">
            <img src={mainScreenImg} alt="match" />
          </div>
          <ul className="tab-container">
            {tabsList.map(eachTab => (
              <TabItem
                ...
                istabActive={whatsIsClicked === eachTab.tabId}
              />
            ))}
          </ul>
          <ul className="image-list-container">
            {searchRes.map(eachImg => (
              <ImageItem
                key={eachImg.id}
                imgDetails={eachImg}
  ...
  render() {
    ...
    const resList = imagesList.filter(
      eachImg => eachImg.category === whatsIsClicked,
    )
